...
def get_batches(arr, batch_size, seq_length):
    ...
    arr = np.array(arr)[:n_batches*batch_size*seq_length]

    # TODO: Reshape into batch_size rows
    arr = np.array(arr).reshape(batch_size, -1)
    
    ## TODO: Iterate over the batches using a window of size seq_length
    for n in range(0, arr.shape[1], seq_length):
        x = arr[:, n:seq_length+n]
        y = x
        y[:, :-1] = arr[:, n+1:seq_length+n]
...
# train_on_gpu = False
if(train_on_gpu):
    print('Training on GPU my MASTER with 30000 Petahash!')
else: 
...
class CharRNN(nn.Module):
    ...
    def __init__(self, tokens, n_hidden=256, n_layers=2,
        ...
        self.fullyconnected = nn.Linear(n_hidden, len(self.chars))
    ...
    def forward(self, x, hidden):
        ...
        y_hat, hidden = self.lstm(x, hidden)
        
        ## TODO: pass through a dropout layer
        out_drop = self.dropout(y_hat)
        
        # Stack up LSTM outputs using view
        # you may need to use contiguous to reshape the output
        outputs = out_drop.contiguous().view(-1, self.n_hidden)
        
        ## TODO: put x through the fully-connected layer
        out = self.fullyconnected(outputs)
...
print(sample(loaded, 100, top_k=5, prime="And Levin said"))
